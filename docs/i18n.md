...
Simplified Chinese (`zh-CN`) is the first shipped translation, and a generated
**pseudo-locale** is used for QA. Requirements live in
...
to a browser**. Right now that is exactly the plain-text error pages served when
the SPA bundle cannot be delivered — three messages. Catalogs are embedded JSON
...
The other server-rendered surface is the shared-task artifacts (`share.html`, the
gist README and description) that Go writes as HTML/Markdown. Those are **not yet
localized**. They will need placeholder interpolation and a way to carry the
locale to code too far from the HTTP layer to hold a `*http.Request` — add a `Tf`
and a context-carried locale alongside the existing helpers when that lands.
...
Locale tags are normalized case-insensitively at both the SPA and Go shell
boundaries, while the canonical catalog name and emitted `<html lang>` retain
BCP 47 casing (for example, `zh-cn` resolves to `zh-CN`).
