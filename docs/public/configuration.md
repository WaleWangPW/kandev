...
| `githubCredentialBroker.reissueSigningKey` | `KANDEV_GITHUB_CREDENTIAL_BROKER_REISSUE_SIGNING_KEY` | random per backend process | Key used to seal repository-exact managed Git credential reissue capabilities. The automatic process-local key lets sessions launched by the current backend recover after a workspace GitHub connection is replaced or re-selected. Set a stable secret when those sessions must also recover across a backend restart. Changing a configured key invalidates outstanding capabilities. |
